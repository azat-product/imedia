 ...
 */
class UsersSocial_ extends Component
{
    ...
    {
        $nID = constant('static::PROVIDER_' . mb_strtoupper($sProviderKey));
        ...
        try {
            if (empty($sProviderKey)) {
                ...
                if (!class_exists('Hybrid_Endpoint')) {
                    require_once modification($hybridPath . 'Hybrid'.DS.'Endpoint.php');
                }
    ...
    public function authData()
    {
        ...
        do {
            ...
            if (!empty($aProfileData['displayName']) && empty($aName)) {
                $aName[] = $aProfileData['displayName'];
            }
     ...
     * @param integer|bool $mProviderID integer - ID провайдера (static::PROVIDER_) или FALSE
     * @return mixed
     */
     ...
     * @param integer|string $nProviderID ID провайдера
     * @param integer $nUserID ID пользователя
     * @return boolean
     */
     ...
     * @param integer|string $nProviderID ID провайдера (static::PROVIDER_)
    ...
    public function getUserSocialAccountsData($nUserID, $bAdminView = false)
    {
        ...
        if ($bAdminView) {
            $configSocial = config::file('social');
            foreach ($aData as $providerID=>&$v) {
                $v['provider_key'] = '';
                if (is_numeric($providerID)) {
                    if (!empty($configSocial['providers']) && is_array($configSocial['providers'])) {
                        foreach ($configSocial['providers'] as $kk => $vv) {
                            if ($this->getProviderID($kk) == $providerID) {
                                $v['provider_key'] = mb_strtolower(strval($providerID));
                                break;
                            }
                        }
                    }
                } else if (is_string($providerID) && !empty($providerID)) {
                    $v['provider_key'] = $providerID;
                }
