...
class ShopsSvcIcon_ extends CImageUploader
{
    # варианты иконок
    const BIG = 'b'; # большая
    const SMALL = 's'; # малая
    ...
    function url($nSvcID, $sFilename, $sVariantKey = self::BIG)
    {
        ...
        if (empty($sFilename)) {
            # иконка-заглушка
            return $this->url . 'default-abonement.png';
        } else {
    ...
    {
        return bff::filter('shops.svc.icon.variants', array(
            'icon_' . self::BIG   => array(
                'title' => _t('shops', 'Иконка (большая)'),
                'key'   => self::BIG,
                'sizes' => array(
