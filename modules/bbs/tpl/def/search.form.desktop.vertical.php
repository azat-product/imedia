...
?>
    <div class="f-asideFilter__element<?= ! empty($button['active']) ? ' selected' : '' ?><?= ! empty($button['hide']) ? ' hide' : '' ?> j-filter-bl" data='<?= ! empty($button['meta']) ? func::php2js($button['meta']) : '' ?>'>
        <a href="#j-f-asideFilter-right-<?= $id ?>" data-toggle="collapse" class="f-asideFilter__element__heading collapsed">
        ...
        <div id="j-f-asideFilter-right-<?= $id ?>" class="collapse j-filter-body">
            <? if(empty($block['ul'])): ?>
                <div class="f-asideFilter__element__in">
                    <?= $items ?>
                    <? if( ! empty($block['reset'])): $disabled = empty($button['active']); ?>
...
$drawCheckbox = function($group, $name, $title = '', $selected = 0, $value = 1, $inSeek = true) use ($lng, $filter_seek) {
    ...
    if( $name !== NULL ) {
        ...
        if( $group ) { ?><div class="f-asideFilter__element j-filter-bl"> <div class="f-asideFilter__element__in<?= ($inSeek ? ' j-seek' : '') ?>"> <? }
                ...
                if ($d['parent'] ) {
                    ...
                    if ($parent_active) # есть выбранные элементы в PARENT-свойстве
                    {
                        ...
                        foreach ($d['multi'] as $k => $m) {
                            ...
                            <div class="f-catfilter__popup__subtitle rel"><span><?= $m['name'] ?></span>
                            <hr/></div><?
            ...
            case Dynprops::typeRange: # Диапазон
            {
                ...
                if ($custom)
                {
                    ...
                    <div class="f-asideFilter__fromto">
                        <input class="span6 fromto-slider-range-min j-from" name="<?= $name ?>[f]" type="text" placeholder="<?= $lng['range_from'] ?>" value="<?= ($from ?: '') ?>" />
                        <input class="span6 fromto-slider-range-max j-to"   name="<?= $name ?>[t]" type="text" placeholder="<?= $lng['range_to'] ?>" value="<?= ($to ?: '') ?>" />
                        <?
...
<div id="j-f-vertical">
    ...
    if( ! empty($extra['price']['enabled']) ) {
        ...
        ?>
            ...
            <div class="f-asideFilter__fromto">
                <input class="span6 fromto-slider-range-min j-from" name="p[f]" type="text" placeholder="<?= $lng['range_from'] ?>" value="<?= ($price_from ?: '') ?>" maxlength="15" />
                <input class="span6 fromto-slider-range-max j-to"   name="p[t]" type="text" placeholder="<?= $lng['range_to'] ?>" value="<?= ($price_to ?: '') ?>" maxlength="15" />
            </div>
        <?
            ...
            if ($nBranches == 1) {
                ...
                foreach ($aMetro['data'] as &$v) {
                    ...
                    <li class="j-metro-branch">
                        <a data-toggle="collapse" data-parent="#j-filterMetro" href="#j-filterMetro<?= $v['id'] ?>"><span><div class="f-catfilter__popup__subtitle__color" style="background: <?= $v['color'] ?>;"><div class="j-cnt"><?= $cnt ? $cnt : '' ?></div></div> <?= $v['t'] ?></span></a>
