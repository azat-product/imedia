...
else if($type == Banners::TYPE_FLASH)
{
    ...
    <a <?= ! empty($target_blank) ? 'target="_blank"' : '' ?> style="display:none;" href="<?= $clickURL; ?>" rel="nofollow" title="<?= $title; ?>" id="<?= $jsID.'i' ?>">
...
{
    if( ! empty($clickURL)){ ?><a <?= ! empty($target_blank) ? 'target="_blank"' : '' ?> title="<?= $title; ?>" href="<?= $clickURL ?>" rel="nofollow"><img src="<?= $showURL ?>" alt="<?= $alt; ?>" <?php if( ! empty($pos_data['height'])){ ?> height="<?= $pos_data['height'] ?>"<?php } ?> /></a>
