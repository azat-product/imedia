...
class DoV2x3x3 extends Migration
{
    protected function init()
    {
        ...
        $this->setSteps([
            ...
            'googlemaps'  => ['t'=>'Карты Google + админ. панель'],
            'textsExtend' => ['t'=>'Расширяем текстовые поля'],
        ]);
    }
    ...
    public function migrate()
    {
        ...
        # Google Maps + Admin
        if ($this->startStep('googlemaps')) {
            $this->table(TABLE_REGIONS_DISTRICTS)
                ->changeColumn('ybounds', 'string', ['limit' => 200, 'default' => ''])
                ->update();
        }

        # Расширяем текстовые поля
        if ($this->startStep('textsExtend')) {
            $this->table(TABLE_SVC)
                ->changeColumn('settings', 'text', ['limit' => MysqlAdapter::TEXT_MEDIUM])
                ->update();
        }
    }
